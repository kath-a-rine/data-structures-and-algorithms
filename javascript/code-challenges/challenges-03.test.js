...
const typeNum = (arr) => {
  let getNum = [];
  arr.filter(num => !isNaN(num) ? getNum.push(num) : null);
  return getNum;
};
//Cody helped identify isNan()
...
const containsAnd = (arr) => {
  let newArr = [];
  arr.filter(words => words.includes('and') ? newArr.push(words): null);
  return newArr;
};
...
const oddValues = (arr) => {
  let getOdds = [];
  arr.filter(num => (num % 2) ? getOdds.push(num) : null);
  return getOdds;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let newArr = [];
  arr.filter(num => !forbiddenValues.includes(num) ? newArr.push(num) : null);
  return newArr;
};
